"""Handler for Elsevier PowerPoint creation.

This module is built on top of python-pptx: http://python-pptx.readthedocs.io

This Module was originally designed for the Journal Update Report, where it is used extensively, however can be easily applied to any PowerPoint reporting pipelines.
"""
# -*- coding: utf-8 -*-
...
from elspy import style as elsevier


class Handler():
	"""Handler with helpful methods to assist in creation of Elsevier powerpoints.
	...
	def create_table(self, table, df,
					 ...
					 header_size=9, header_bold=True, header_italic=False, header_text_color=elsevier.RGB.white, header_color=elsevier.RGB.orange, merge_header=None,
					 index_size=9, index_bold=False, index_italic=False, index_text_color=elsevier.RGB.grey_dark,
					 totals_size=9, totals_bold=False, totals_italic=False, totals_text_color=elsevier.RGB.grey_dark,
					 text_size=9, text_bold=False, text_italic=False, text_color=elsevier.RGB.grey_dark,
			...
			for level in range(df.columns.nlevels):
				for i in range(num_cols):
					...
					try:
						...
						r.font.name = elsevier.Font.name
					except IndexError:
				...
				for i in range(num_rows):
					...
					if banded_rows:
						if not keep_header_formatting:
							c.fill.solid()
							c.fill.fore_color.rgb = RGBColor(*elsevier.RGB.grey_light) if (i-df.columns.nlevels) % 2 == 0 else RGBColor(*elsevier.RGB.grey_light2)
					tf = c.text_frame
					p = tf.paragraphs[0]
					try:
						r = p.runs[0]
		...
		for row in range(df.shape[0]):
			for col in range(df.shape[1]):
				...
				if banded_rows:
					c.fill.solid()
					c.fill.fore_color.rgb = RGBColor(*elsevier.RGB.grey_light) if row % 2 == 0 else RGBColor(*elsevier.RGB.grey_light2)
				tf = c.text_frame
				p = tf.paragraphs[0]
				if df.columns[col] in column_alignment_map:
					p.alignment = column_alignment_map[df.columns[col]]
	...
	def create_chart(self, chart, df,
					 ...
					 chart_title=None, chart_title_text_size=12, chart_title_text_bold=False, chart_title_text_color=elsevier.RGB.grey_dark,
					 axis_label_text_size=10, axis_label_text_bold=False, axis_label_text_color=elsevier.RGB.grey_dark,
					 axis_text_size=10, axis_text_bold=False, axis_text_color=elsevier.RGB.grey_dark,
					 data_labels=True, data_label_position=None, data_label_rotate=False,
					 data_label_text_size=8, data_label_text_bold=False, data_label_text_color=elsevier.RGB.grey_dark,
					 chart_legend=True, legend_in_layout=False, legend_position=pptx.enum.chart.XL_LEGEND_POSITION.BOTTOM,
					 legend_text_size=10, legend_text_bold=False, legend_text_color=elsevier.RGB.grey_dark,
		...
		if chart_legend:
			...
			chart.legend.font.name = elsevier.Font.name
		...
		if data_labels:
			...
			plot.data_labels.font.name = elsevier.Font.name
			...
			try:
				if chart_type == pptx.enum.chart.XL_CHART_TYPE.PIE:
					for i, slice in enumerate(series.points):
						...
						slice.format.fill.fore_color.rgb = RGBColor(*elsevier.RGB.colorbar[i])
				if chart_type == pptx.enum.chart.XL_CHART_TYPE.LINE:
					series.format.line.width = line_width
					series.format.line.color.rgb = RGBColor(*elsevier.RGB.colorbar[i])
					if not highlight_line is None and i == df.columns.get_loc(highlight_line):
						series.format.line.width = line_width * 2
				else:
					series.format.fill.solid()
					series.format.fill.fore_color.rgb = RGBColor(*elsevier.RGB.colorbar[i])
					series.format.line.color.rgb = RGBColor(*elsevier.RGB.white)
			except IndexError:
		...
		try:
			...
			chart.value_axis.tick_labels.font.name = elsevier.Font.name
			...
			chart.category_axis.tick_labels.font.name = elsevier.Font.name
			if number_format:
