"""Handler for Elsevier Word Document creation.

This module is built on top of python-docx: https://python-docx.readthedocs.io/

This Module was designed for the Journal Update Report, where it is used extensively, however can be easily applied to any Word Document reporting pipelines.
"""
# -*- coding: utf-8 -*-
...
class Handler():
	"""Handler with helpful methods to assist in creation of Elsevier documents
	...
	def create_table(self, doc, df,
					 ...
					 header_size=8, header_bold=True, header_italic=False, header_text_color=elsevier.RGB.grey_dark, header_color=None, merge_header=None,
					 index_size=8, index_bold=False, index_italic=False, index_text_color=elsevier.RGB.grey_dark,
					 totals_size=8, totals_bold=False, totals_italic=False, totals_text_color=elsevier.RGB.grey_dark,
					 text_size=8, text_bold=False, text_italic=False, text_color=elsevier.RGB.grey_dark,
				...
				for i in range(num_rows):
					...
					if banded_rows:
						if not keep_header_formatting:
							rgb = RGBColor(*elsevier.RGB.grey_light) if (i-df.columns.nlevels) % 2 == 0 else RGBColor(*elsevier.RGB.grey_light2)
							xml_shd = docx.oxml.parse_xml(r'<w:shd {} w:fill="{}"/>'.format(docx.oxml.ns.nsdecls('w'), rgb))
							c._tc.get_or_add_tcPr().append(xml_shd)
					try:
		...
		for row in range(df.shape[0]):
			for col in range(df.shape[1]):
				...
				c.margin_right = docx.shared.Inches(margins_master[cell_margins]['right'])
				if banded_rows:
					rgb = RGBColor(*elsevier.RGB.grey_light) if row % 2 == 0 else RGBColor(*elsevier.RGB.grey_light2)
					xml_shd = docx.oxml.parse_xml(r'<w:shd {} w:fill="{}"/>'.format(docx.oxml.ns.nsdecls('w'), rgb))
					c._tc.get_or_add_tcPr().append(xml_shd)
				p = c.paragraphs[0]
				if df.columns[col] in column_alignment_map:
